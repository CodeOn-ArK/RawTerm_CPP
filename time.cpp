...
void print_time(void) {
    ...
    /* This is a workaround as `struct tm' from <ctime> doesn't seem to have a field for milliseconds. Here
     * time_since_epoch() will give us the unix epoch time which has been constantly ticking since 00:00:00 UTC on
     * 1 January 1970 (an arbitrary date) and is therefore a huge number. We're interested in only extracting the
     * milliseconds following the last second, so we first represent it fully in milliseconds and then take out the
     * number of milliseconds uptil the last second so we'll be left with just the number of milliseconds following the
     * last second.
     */
